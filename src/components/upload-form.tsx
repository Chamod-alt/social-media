...
export function UploadForm({ post, onPostUpdated, onFormSubmit }: UploadFormProps) {
  ...
  const form = useForm<UploadValues>({
    ...
    defaultValues: { caption: post?.caption || "", image: undefined },
  });
  ...
  const onSubmit = async (values: UploadValues) => {
    ...
    try {
      ...
      if (values.image?.[0]) {
        ...
        if (!imgbbResponse.ok) throw new Error("Image upload failed. Please try again.");
      ...
      if (isEditing && post?.id) {
        ...
        await update(postRef, { caption: values.caption || post.caption, imageUrl });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FormField control={form.control} name="caption" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Textarea placeholder="What's on your mind?" {...field} className="resize-none" /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
    ...
    try {
      ...
      if (isEditing && post.id) {
        ...
        if(values.image?.[0]){
            ...
            const imgbbResponse = await fetch(
            `https://api.imgbb.com/1/upload?key=f3ec8b16a01772d8dc660b2542e711e4`,
            {
                method: "POST",
                body: formData,
            }
